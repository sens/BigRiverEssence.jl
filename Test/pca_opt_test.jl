# test/pca_opt_test.jl — formal tests for pca_opt ONLY (no dependence on pca).
# Without the original to diff against, every check rests on intrinsic truth:
# output invariants, ground-truth recovery, :svd-vs-:cov internal agreement,
# the sign-consistency contract, transform/inverse round-trips, argument checks.

# ---- helpers ----
# loadings match up to per-column sign → compare via abs, worst column
...
@testset "output structure & invariants" begin
    ...
    # loadings are unit-norm columns (orthonormal directions)
    for j in 1:k
    ...
    # mean/scale recover the column statistics actually used
    @test isapprox(m.mean, vec(mean(X, dims=1)); atol=1e-10)
    @test all(m.scale .== 1.0)                       # no standardize → ones
    ms = pca_opt(X; k=k, standardize=true)
    @test isapprox(ms.scale, vec(std(X, dims=1)); atol=1e-10)
end

@testset "loadings are orthonormal" begin
    # PCA directions must be mutually orthogonal as well as unit-norm:
    # Vᵀ V = I_k.  (Holds for both :svd and :cov.)
    Random.seed!(21)
    n, p, k = 300, 15, 6
    X = randn(n, p)
    for method in (:svd, :cov)
        m = pca_opt(X; k=k, method=method)
        G = m.loadings' * m.loadings
        @test isapprox(G, I(k); atol=1e-8)
    end
end
...
@testset "ground-truth: recovers a planted direction" begin
    ...
    for method in (:svd, :cov)
        m = pca_opt(X; k=2, method=method)
        @test abs(dot(m.loadings[:, 1], w)) > 0.999      # PC1 aligns with w
        @test m.propOFvar[1] > 0.99                      # PC1 dominates variance
    end
end

@testset "ground-truth: variances equal the data's spectrum" begin
    # for centered X, the PCA variances must equal svdvals(Xc).^2/(n-1).
    # this checks the *values*, not just that they're sorted.
    Random.seed!(31)
    n, p, k = 400, 12, 12
    X = randn(n, p)
    Xc = X .- mean(X, dims=1)
    truevars = (svdvals(Xc) .^ 2) ./ (n - 1)             # the exact PCA spectrum
    for method in (:svd, :cov)
        m = pca_opt(X; k=k, method=method)
...
@testset ":svd and :cov agree (internal cross-check)" begin
    # the two methods compute the same PCA two ways → they must agree.
    # this is the consistency anchor that replaces opt-vs-orig here.
    Random.seed!(3)
    n, p, k = 300, 20, 8
    X = randn(n, p)
    msvd = pca_opt(X; k=k, method=:svd)
    mcov = pca_opt(X; k=k, method=:cov)
...
@testset "proportions sum correctly" begin
    # with all components kept, propOFvar must sum to 1 (all variance accounted).
    Random.seed!(17)
    n, p = 200, 10
    X = randn(n, p)
    m = pca_opt(X; k=p, method=:svd)
...
@testset "standardize round-trip" begin
    ...
    m = pca_opt(X; k=10, standardize=true)
    scores = pca_transform(m, X)
    Xrec   = pca_invtransform(m, scores)
...
@testset "determinism" begin
    # pca_opt has no random component → identical results on repeat calls.
    Random.seed!(99)
    X = randn(180, 14)
    a = pca_opt(X; k=6, method=:svd)
    b = pca_opt(X; k=6, method=:svd)
...
@testset "argument validation" begin
    Random.seed!(0)
    X = randn(50, 8)
    @test_throws ArgumentError pca_opt(X; k=0)              # k below range
    @test_throws ArgumentError pca_opt(X; k=9)             # k > min(n,p)=8
    @test_throws ErrorException pca_opt(X; method=:bogus)  # unknown method
end