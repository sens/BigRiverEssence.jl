...
@testset "matches R PMA::PMD (offline reference fixtures)" begin
	...
	# Test/Data/PMD/ (generated by generate_pmd_reference.R). Offline so the suite doesn't need a live R;
	...
	if !isfile(joinpath(refdir, "X.csv"))
		@info "PMD R-reference fixtures not found; skipping. Run generate_pmd_reference.R to create them."
	else
