@testset "internal: SignConsistency_opt! (largest-|entry| made positive)" begin
	...
	BigRiverEssence.SignConsistency_opt!(V)
	...
	BigRiverEssence.SignConsistency_opt!(V2)
	...
	V3 = [-5.0  2.0
		 1.0 -7.0
		 3.0  4.0]
	BigRiverEssence.SignConsistency_opt!(V3)
	...
	@test BigRiverEssence.SignConsistency_opt!(V4) === V4

	# idempotent: applying twice == applying once (pivot already positive 2nd time)
	V5    = randn(8, 4)
	once  = BigRiverEssence.SignConsistency_opt!(copy(V5))
	twice = BigRiverEssence.SignConsistency_opt!(BigRiverEssence.SignConsistency_opt!(copy(V5)))
	@test once == twice

	# the operation only changes signs, never magnitudes
	V6 = randn(6, 3);
	before = abs.(copy(V6))
	BigRiverEssence.SignConsistency_opt!(V6)
	@test abs.(V6) == before

	# all-zero column ⇒ sign(0)=0 guard leaves it untouched (no NaN/Inf)
	V7 = reshape([0.0, 0.0, 0.0], 3, 1)
	BigRiverEssence.SignConsistency_opt!(V7)
	@test all(iszero, V7)
